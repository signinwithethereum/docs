...
Represents a parsed or constructed [EIP-4361](https://eips.ethereum.org/EIPS/eip-4361) message. Build one directly from fields, via [`NewMessage`](#newmessagedomain-address-uri-nonce-options), or parse from an EIP-4361 string via [`ParseMessage`](#parsemessages-string-message-error).
...
#### `ParseMessage(s string) (*Message, error)`
...
#### `NewMessage(domain, address, uri, nonce, options)`
...
#### `(*Message).VerifyEIP191(signature string) (*ecdsa.PublicKey, error)`
...
Optional binding checks performed during `VerifyWith`.
...
| `URI`       | `*string`   | Expected URI — required in strict mode                                        |
| `ChainID`   | `*int`      | Expected chain ID — required in strict mode                                   |
...
| `Strict`           | `bool`                       | When `true`, `URI` and `ChainID` are required in `VerifyParams`       |
...
| `ErrInvalidStatement`      | Statement contains invalid characters (non-printable / LF / etc.)    |
...
| `ErrMissingURI`            | `URI` binding is required in strict mode                             |
| `ErrMissingChainID`        | `ChainID` binding is required in strict mode                         |
...
### `ContractSignatureVerifier` Interface
...
Pass `Strict: true` to require `URI` and `ChainID` bindings, for full contextual binding:

```go
domain, nonce, uri := "example.com", storedNonce, "https://example.com"
chainID := 1
