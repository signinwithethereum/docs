...
Smart-wallet verification uses a built-in JSON-RPC client backed by the standard library's `net/http` — no additional Ethereum RPC gem is required (though you can plug in your own; see [Smart Contract Wallets](#smart-contract-wallets-eip-1271--eip-6492)).

## API Reference

### `Siwe::Message` Class

Represents a parsed or constructed [EIP-4361](https://eips.ethereum.org/EIPS/eip-4361) message. Build one directly via [`Siwe::Message.new`](#construction), or parse from an EIP-4361 string via [`Siwe::Message.parse`](#siwemessageparsestr---message). Instances are **frozen** (immutable) once constructed.
...
Reconstruct a message from the JSON produced by [`#to_json`](#to_h---hash--to_json---string):
...
Verifies the message against a signature and checks time-based validity plus any field constraints you supply. **Never raises on verification failure** — it returns a [`Siwe::Response`](#siweresponse) whose `#error` carries the failure. Use [`#verify!`](#verify---self) when you'd rather rescue an exception.
...
Same arguments and verification as [`#verify`](#verify---siweresponse), but raises the [`Siwe::Error`](#error-handling) on failure and returns `self` on success — the idiomatic path for a Rails / Sinatra controller:
...
The value object returned by [`#verify`](#verify---siweresponse). A `Data` type with three readers:
