...
A `pydantic.BaseModel` representing an [EIP-4361](https://eips.ethereum.org/EIPS/eip-4361) message. Construct one directly from fields, or parse from an EIP-4361 string via [`from_message`](#from_messagemessage-str-abnf-bool--true---siwemessage).
...
Verifies the message against a signature and checks time-based validity and any field constraints you supply. Returns `None` on success; raises a [`VerificationError`](#verificationerror) subclass on failure.
...
| `strict`     | `bool`                   | When `True`, `domain`, `uri`, and `chain_id` are required                                    |
...
Pass a `web3` provider to `verify()` to enable on-chain signature validation for smart contract wallets:
...
`strict=True` requires `domain`, `uri`, and `chain_id` to be provided as verification parameters, for full contextual binding:
