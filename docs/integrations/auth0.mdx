...
Auth0 connects to a SIWE [OIDC Provider](/oidc-provider) as a custom social connection:
...
You need a running instance of the [SIWE OIDC Provider](/oidc-provider). See the [Deployment Guide](/oidc-provider/deployment-guide) for options.
...
- [OIDC Provider Documentation](/oidc-provider) — deploy and configure the identity provider
- [OIDC API Reference](/oidc-provider/api-reference) — endpoints, scopes, and claims
